...
- MT6000 `LAN` trunk -> CRS310 `ether1` (native management VLAN 99, tagged VLAN 10/20/30/40/50).
- CRS310 `ether2` trunk -> office MT3000 uplink over cable #1 (native management VLAN 99, tagged VLAN 20/30/40/50).
- CRS310 `ether3` access VLAN 20 -> office wired devices over cable #2.
...
- MT6000 Wi-Fi radios are redundant in garage placement and should remain disabled.
...
flowchart LR
  ...
  MT6000 -->|"2.5G trunk (native 99, tagged 10/20/30/40/50)"| CRS_E1["CRS310 ether1"]
  CRS_E1 --> CRS["MikroTik CRS310 (Garage Core Switch)"]

  CRS -->|"ether2 trunk to office AP (native 99, tagged 20/30/40/50)"| AP["GL.iNet Beryl (MT3000) Office AP"]
  ...
  CRS -->|"ether3 access VLAN20"| OFFICE_WIRED["Office wired/printer via cable #2"]
